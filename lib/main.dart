...
class _CameraAwesomeAppState extends State<CameraAwesomeApp> {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(title: 'camerAwesome', home: CameraPage());
  }
}
  ...
  Widget build(BuildContext context) {
    Future<bool> savetoGallary(MediaCapture media) async {
      ...
      if (media.isPicture) {
        ...
        var path = await encodeMessage(media: media, msg: imei);

        result = await GallerySaver.saveVideo(
            albumName: 'tracex', path, toDcim: true);
      }
